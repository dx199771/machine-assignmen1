/*
	Search.java - abstract class specialising to RamblersSearch etc
**  2013 version
*/
  ...
  public  String runSearch (SearchState initState, String strat) {
    ...
    private void expand () {
      ...
      for (SearchNode snode: successorNodes){
		  ...
		  SearchState s3 = snode.get_State();
		  String s4 = s3.toString();
          //the cost is higher point minus lower point plus 1 if is going up
          //the cose is 1 if is going down 
	  if(tm.getTmap()[Integer.parseInt(s2.substring(0,s2.indexOf(",")))][Integer.parseInt(s2.substring(s2.indexOf(",")+1))]-tm.getTmap()[Integer.parseInt(s4.substring(0,s4.indexOf(",")))][Integer.parseInt(s4.substring(s4.indexOf(",")+1))]>=0){
		  snode.setLocalCost(1);
